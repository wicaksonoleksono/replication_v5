...
class aggregation_ihc:
    ...
    def _integrate_stages(self):
        ...
        stg2_total = stg2_total.drop(["implicit_class_x", "extra_implicit_class_x",
                                     "implicit_class_y", "extra_implicit_class_y"], axis=1)
        ...
        stg1_implicit_stg2_stg3_inner_total = pd.merge(
            left=stg1_implicit_stg2_inner_total, right=stg3_total, how="inner", on="ID")
        ...
        train, valid, test = np.split(pure_set.sample(frac=1, random_state=42), [
                                      int(.6*len(pure_set)), int(.8*len(pure_set))])
